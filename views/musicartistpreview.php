...
//print_r($arow); die;
                                    ...
                                    <div class="col-md-9 col-xs-12 wi70">
                                        ...
                                        <div class="home-cont-row">
                                            ...
                                            <?php
                                            //echo "SELECT * FROM tbl_user_images where id=" . $_REQUEST['workid'];
                                            $qrya = mysql_query("SELECT * FROM tbl_user_images where id=" . $_REQUEST['workid']);
                                            $dataq = mysql_fetch_assoc($qrya);
                                            $arraysel = array();
                                          
                                            $suers = explode(",",$dataq['music_artist']);
                                            
                                            for($i=0;$i < count($suers);$i++){
                                               
                                                $qrya = mysql_query("SELECT * FROM tbl_users where id=" . $suers[$i]);
                                                $catdata = mysql_fetch_assoc($qrya);
                                                if($catdata['image']!=''){
                                                    if (is_file($_SESSION['SITE_IMG_PATH'] . "artist/" . $catdata['image'])) {
                                                        $userimage = $_SESSION['FRNT_DOMAIN_NAME'] . "upload/artist/" . $catdata['image'];
                                                    } else {
