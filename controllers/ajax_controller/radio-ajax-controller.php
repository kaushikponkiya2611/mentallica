<?php 
	...
	if(isset($_POST['view']) && $_POST['view'] != ''){
		$id = $_POST['id'];
		$qry = "SELECT * FROM tbl_music where id = '".mysql_real_escape_string($id)."'";
	...
	if(isset($_POST['statusactive']) && $_POST['statusactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_music SET status = 1 WHERE id = ".mysql_real_escape_string($val);
	...
	if(isset($_POST['statusinactive']) && $_POST['statusinactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_music SET status = 0 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_music SET status = 2 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST['hid_add']) && $_POST['hid_add'] != ''){
		$qry = "SELECT * FROM tbl_music WHERE title = '".mysql_real_escape_string(trim($_POST['txt_addtitle']))."' and status!=2 ";
		$result = $modelObj -> fetchRow($qry);
		...
		if($flag == 1)
		{
			...
			if(!dir($upload_dirthumb))
			{
				...
				}$qry = "INSERT INTO tbl_music (title,image,time,start_time,end_time,weight,cr_date,status) VALUES ('".clear_input($_POST['txt_addtitle'])."','".$insertimage."','".clear_input($_POST['txt_addtime'])."','".clear_input($_POST['txt_addstart_time'])."','".clear_input($_POST['txt_addend_time'])."','".clear_input($_POST['txt_addweight'])."',NOW(),1)";
			$result = $modelObj->runQuery($qry);
			echo "1";
	   }
	   else
	...
	if(isset($_POST['statusid']) && $_POST['statusid'] != ''){
		$qry = "UPDATE  tbl_music SET status = ".mysql_real_escape_string($_POST['status'])." WHERE id = ".mysql_real_escape_string($_POST['statusid']);
		$result =  $modelObj->runQuery($qry);
	}
	?>
	<?php
	if(isset($_POST['hid_update']) && $_POST['hid_update'] != ''){
		$qry = "SELECT * FROM tbl_music WHERE title = '".mysql_real_escape_string(trim($_POST['txt_addtitle']))."' and status!=2 and id != '".$_POST['hid_userid']."'";
		$result = $modelObj -> fetchRow($qry);
		...
		{if(isset($_FILES["txt_addimage"]["tmp_name"]))
				{
					...
					if($_FILES["txt_addimage"]["tmp_name"] != '')
					{
						...
						$qry_img="UPDATE  tbl_music SET image='".$imgname."' where id='".mysql_real_escape_string($_POST['hid_userid'])."' ";
						$res_img= $modelObj->runQuery($qry_img);
					}
				}$qry = "UPDATE tbl_music SET title = '".clear_input($_POST['txt_addtitle'])."',time = '".clear_input($_POST['txt_addtime'])."',start_time = '".clear_input($_POST['txt_addstart_time'])."',end_time = '".clear_input($_POST['txt_addend_time'])."',weight = '".clear_input($_POST['txt_addweight'])."'WHERE id = '".mysql_real_escape_string($_POST['hid_userid'])."'";
			$result = $modelObj->runQuery($qry);
			echo "1";
		}
		else
	...
	if(isset($_POST['viewdiv']) && $_POST['viewdiv'] != ''){
		...
		{if(trim($_POST['txt_srctitle']) != '')
			...
			{
				$qry = "SELECT * FROM tbl_music where status!=2 ".$_SESSION['srchqry']." order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_music where status!=2 ".$_SESSION['srchqry']." ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}
			$qry11 = mysql_query("SELECT * FROM tbl_music where status!=2 ".$_SESSION['srchqry']."")or die(mysql_error());
		}
		else
		{
			if($_POST['fieldname'] == '0')
			{
				$qry = "SELECT * FROM tbl_music where status!=2 order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_music where status!=2 ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}	  
			$qry11 = mysql_query("SELECT * FROM tbl_music where status!=2 ")or die(mysql_error());
		}
	...
	if(isset($_POST['delete']) && $_POST['delete'] != ''){
			$qry = "UPDATE tbl_music SET status = 2 WHERE id = '".$_POST['id']."'";
			$result = $modelObj->runQuery($qry);
		  if($result){
			echo $successmsg = '1';
		  }else{
	...
	if(isset($_POST['edit']) && $_POST['edit'] != ''){
		$qry = "SELECT * FROM tbl_music WHERE id = '".$_POST['id']."'";
