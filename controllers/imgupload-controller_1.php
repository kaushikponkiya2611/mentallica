...
class ImguploadController extends CommonController {
            ...
            if (isset($_FILES["file_img_upload"]["tmp_name"])) {
                ...
                if ($_FILES["file_img_upload"]["tmp_name"] != '') {
                    ...
                    if (move_uploaded_file($tmpfile, $upload_dir . $insertimage)) {
                        ...
                        $file_size1 = ceil($file_size1 / 1024); // Get file size in KB
                        ...
                        if ($result_plan['image_limit'] > 0) {
                            ...
                            $cutcoins = ceil($file_size / $kbtocoins);
                        }
                        //K- Image Rules
                        $arr = array();
                        ...
                        $usr_insert_img = $this->insertuserimage($insertimage, $_POST['txt_img_title'],$_POST['txt_img_text'] ,$image_rules, $_POST['sel_currency'], $_POST['txt_img_price'], $_POST['txt_dwn_price'], $_POST['sel_category'], $this->userdetail['image_limit'], $targetFile, $_POST['sel_subcategory'], $cutcoins);
                    }
                }
            }
    ...
    function insertuserimage($insertimage, $imgtitle, $imgtext,$image_rules, $currency, $imgprice, $imgdwnprice, $catid, $imglimit, $musicnewname, $subcatid, $cutcoins) {
		if(empty($imgdwnprice)){
			$imgdwnprice = 00.00;
		}
        $qry = "INSERT INTO `tbl_user_images`(`user_id`, `image`,`music` ,`img_title`, `image_text`,`image_rules`, `price_currency`, `img_price`,`dowanload_price`, `category_id`, `cr_date`, `status`) VALUES ('" . $_SESSION['po_userses']['flc_usrlogin_id'] . "','" . clear_input($insertimage) . "','" . $musicnewname . "','" . clear_input($imgtitle) . "','" . clear_input($imgtext) . "','".$image_rules."','" . clear_input($currency) . "','" . clear_input($imgprice) . "','" . clear_input($imgdwnprice) . "','" . clear_input($catid) . "',NOW(),1)";
        $result = $this->modelObj->runQuery($qry);

        $lastid = mysql_insert_id();
        if ($lastid && $lastid > 0):
            $imglimit = $imglimit - 1;
            $qry2 = "UPDATE tbl_users SET image_limit = image_limit-" . clear_input($cutcoins) . " WHERE status = 1 AND id = " . $_SESSION['po_userses']['flc_usrlogin_id'];
