...
class CommonController {
    ...
    function __construct() {
        ...
        if ((isset($_SESSION['po_userses']['flc_usrlogin_id']) && $_SESSION['po_userses']['flc_usrlogin_type'] != 1) && ($_GET['pid'] == 'imgupload' || $_GET['pid'] == 'preview' || $_GET['pid'] == 'artistsimages')) {
            $this->redirecttohomepage();
        }
        /* Ends - Restrict Artist Pages */

        if ((!isset($_SESSION['po_userses']['flc_usrlogin_id']) || $_SESSION['po_userses']['flc_usrlogin_id'] == '') && ($_GET['pid'] == 'profile' || $_GET['pid'] == 'imgupload' || $_GET['pid'] == 'preview' || $_GET['pid'] == 'artistsimages' || $_GET['pid'] == 'editimage')) {
            $this->redirecttohomepage();
        }
    ...
    function getuserimages() {
        $qry = "SELECT * FROM tbl_user_images WHERE status=1 AND user_id = '" . $_SESSION['po_userses']['flc_usrlogin_id'] . "' ORDER BY cr_date DESC";
    ...
    function getuserimageinactivelistbycatid() {
        $qry = "SELECT * FROM tbl_user_images WHERE status=0 AND user_id = '" . $_SESSION['po_userses']['flc_usrlogin_id'] . "' ORDER BY image_rank ASC";
        return $result = $this->modelObj->fetchRows($qry);
    }
    ...
    function getPagetTitleAndHeading() {
        if ($_GET['pid'] == 'home') {
            ...
            $h1 = "Newsletter";
            $image = "default.png";
		} else if ($_GET['pid'] == 'internetradio') {
