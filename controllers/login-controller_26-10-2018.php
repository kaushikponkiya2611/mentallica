...
class LoginController extends CommonController {
    ...
    function checkLoginDetails($emailid, $password, $type) {
        ...
        /*if($type=='1'){
            $qry = "select * from tbl_users where status=1 and emailid = '" . clear_input($emailid) . "' and password = '" . clear_input(md5($password)) . "' and usertype = '" . clear_input($type) . "' ";
            ...
            $qry = "select * from tbl_users where status=1 and emailid = '" . clear_input($emailid) . "' and password = '" . clear_input(md5($password)) . "' and usertype = '1' and is_paypal_user = 'yes'";
            $result = $this->modelObj->fetchRow($qry);
           
            if ($result['id'] != '' && $result['status'] == 1) {
                $qry1 = "select * from tbl_users where status=1 and emailid = '" . clear_input($emailid) . "' and password = '" . clear_input(md5($password)) . "' and usertype = '3' and is_paypal_user = 'yes'";
                $result1 = $this->modelObj->fetchRow($qry1);
            ...
            $qry = "select * from tbl_users where status=1 and emailid = '" . clear_input($emailid) . "' and password = '" . clear_input(md5($password)) . "' and usertype = '3' and is_paypal_user = 'yes'";
            $result = $this->modelObj->fetchRow($qry);
        }*/
    ...
    function checkPaypalLogin($emailid, $password, $type,$data,$is_paypal){
        $is_paypal_cond = '';
        if($is_paypal=='yes'){
