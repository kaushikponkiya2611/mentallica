...
if(!file_exists($ourFileName5))
{
	...
	if(isset($_POST[\'view\']) && $_POST[\'view\'] != \'\'){
		$id = $_POST[\'id\'];
		$qry = "SELECT * FROM '.$mytables.' where id = \'".mysql_real_escape_string($id)."\'";
	...
	if(isset($_POST[\'statusactive\']) && $_POST[\'statusactive\'] != \'\')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  '.$mytables.' SET status = 1 WHERE id = ".mysql_real_escape_string($val);
	...
	if(isset($_POST[\'statusinactive\']) && $_POST[\'statusinactive\'] != \'\')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  '.$mytables.' SET status = 0 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST[\'deleselected\']) && $_POST[\'deleselected\'] != \'\'){
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  '.$mytables.' SET status = 2 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST[\'hid_add\']) && $_POST[\'hid_add\'] != \'\'){
		$qry = "SELECT * FROM '.$mytables.' WHERE '.$myarray[0]['Field'].' = \'".mysql_real_escape_string(trim($_POST[\'txt_add'.$myarray[0]['Field'].'\']))."\' and status!=2 ";
		$result = $modelObj -> fetchRow($qry);
		...
		foreach($myarray as $k => $tablefields)
		{
			...
			foreach($myarray as $k => $tablefields)
			{
				if($tablefields['Field']=='password')
				{
					$stringData .='\'".mysql_real_escape_string(trim(md5($_POST[\'txt_add'.$tablefields['Field'].'\'])))."\',';
				}
	...
	if(isset($_POST[\'statusid\']) && $_POST[\'statusid\'] != \'\'){
		$qry = "UPDATE  '.$mytables.' SET status = ".mysql_real_escape_string($_POST[\'status\'])." WHERE id = ".mysql_real_escape_string($_POST[\'statusid\']);
		$result =  $modelObj->runQuery($qry);
	}
	?>
	<?php
	if(isset($_POST[\'hid_update\']) && $_POST[\'hid_update\'] != \'\'){
		$qry = "SELECT * FROM '.$mytables.' WHERE '.$myarray[0]['Field'].' = \'".mysql_real_escape_string(trim($_POST[\'txt_add'.$myarray[0]['Field'].'\']))."\' and status!=2 and id != \'".$_POST[\'hid_userid\']."\'";
		$result = $modelObj -> fetchRow($qry);
		...
		foreach($myarray as $k => $tablefields)
		{
			if(preg_match("/image/i", $tablefields['Field']))
			{
				$stringData .= 'if(isset($_FILES["txt_add'.$tablefields['Field'].'"]["tmp_name"]))
				{
					...
					if($_FILES["txt_add'.$tablefields['Field'].'"]["tmp_name"] != \'\')
					{
						...
						$qry_img="UPDATE  '.$mytables.' SET '.$tablefields['Field'].'=\'".$imgname."\' where id=\'".mysql_real_escape_string($_POST[\'hid_userid\'])."\' ";
			...
			$stringData .='WHERE id = \'".mysql_real_escape_string($_POST[\'hid_userid\'])."\'";
			$result = $modelObj->runQuery($qry);
			echo "1";
		}
		else
	...
	if(isset($_POST[\'viewdiv\']) && $_POST[\'viewdiv\'] != \'\'){
		...
		{';
			foreach($myarray as $k => $tablefields)
			{
				if($tablefields['Field'] != 'password')
				{
					...
					else
					{
						...
						{
							$searchqry .="and '.$tablefields['Field'].' LIKE \'%".mysql_real_escape_string(trim($_POST[\'txt_src'.$tablefields['Field'].'\']))."%\'";
			...
			$qry11 = mysql_query("SELECT * FROM '.$mytables.' where status!=2 ".$_SESSION[\'srchqry\']."")or die(mysql_error());
		}
		else
		{
			if($_POST[\'fieldname\'] == \'0\')
			...
			$qry11 = mysql_query("SELECT * FROM '.$mytables.' where status!=2 ")or die(mysql_error());
		}
