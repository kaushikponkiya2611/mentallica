<?php 
	...
	if(isset($_POST['view']) && $_POST['view'] != ''){
		$id = $_POST['id'];
		$qry = "SELECT * FROM tbl_users where id = '".mysql_real_escape_string($id)."' and usertype=1";
		$result = $modelObj->fetchRow($qry);

		$qry = "SELECT * FROM tbl_plans where status!=2 AND id='".clear_input($result['plan_id'])."' LIMIT 1";
		  	$plndtl = $modelObj->fetchRow($qry);
	?>
	<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:13px"><tr class="white_bg">
						...
						<td width="120" align="right" class="popup_listing_border"><strong>Plan Selected:</strong></td>
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border"><?php echo stripslashes($plndtl['plan_name']) ?></td>
					  </tr><tr class="light_bg">
						<td width="120" align="right" class="popup_listing_border"><strong>Remaining upload limit:</strong></td>
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border"><?php echo stripslashes($result['image_limit']) ?></td>
					  </tr></table>
	<?php 
	} 
	?>			
	...
	if(isset($_POST['hid_add']) && $_POST['hid_add'] != ''){
		$qry = "SELECT * FROM tbl_users WHERE emailid = '".mysql_real_escape_string(trim($_POST['txt_addemailid']))."' and status!=2 ";
		$result = $modelObj -> fetchRow($qry);
		
		if(strtolower(trim($result['emailid'])) == strtolower(trim($_POST['txt_addemailid'])))
		{
			$flag = 0;
		}
		else
		...
		if($flag == 1)
		{
			...
			if(!dir($upload_dirthumb))
			{
				...
				$qry = "INSERT INTO tbl_users (ref_id,first_name,last_name,emailid,password,mobileno,image,plan_id,cr_date,status) VALUES ('".clear_input($ref_code)."','".clear_input($_POST['txt_addfirst_name'])."','".clear_input($_POST['txt_addlast_name'])."','".clear_input($_POST['txt_addemailid'])."','".mysql_real_escape_string(trim(md5($_POST['txt_addpassword'])))."','".clear_input($_POST['txt_addmobileno'])."','".$insertimage."','".clear_input($_POST['txt_addplan_id'])."',NOW(),1)";
			$result = $modelObj->runQuery($qry);
			echo "1";
	   }
	   else
	...
	if(isset($_POST['hid_update']) && $_POST['hid_update'] != ''){
		$qry = "SELECT * FROM tbl_users WHERE emailid = '".mysql_real_escape_string(trim($_POST['txt_addemailid']))."' and status!=2 and id != '".$_POST['hid_userid']."'";
		$result = $modelObj -> fetchRow($qry);
		...
		if(!dir($upload_dir))
			...
			if(isset($_FILES["txt_addimage"]["tmp_name"]))
				...
				}$qry = "UPDATE tbl_users SET ref_id = '".clear_input($_POST['txt_addref_id'])."',first_name = '".clear_input($_POST['txt_addfirst_name'])."',last_name = '".clear_input($_POST['txt_addlast_name'])."',emailid = '".clear_input($_POST['txt_addemailid'])."',mobileno = '".clear_input($_POST['txt_addmobileno'])."',plan_id = '".clear_input($_POST['txt_addplan_id'])."'WHERE id = '".mysql_real_escape_string($_POST['hid_userid'])."'";
			$result = $modelObj->runQuery($qry);
			echo "1";
		}
		else
	...
	if(isset($_POST['viewdiv']) && $_POST['viewdiv'] != ''){
		...
		{if(trim($_POST['txt_srcref_id']) != '')
			...
			{
				$qry = "SELECT * FROM tbl_users where status!=2 and usertype=1 ".$_SESSION['srchqry']." order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_users where status!=2 and usertype=1 ".$_SESSION['srchqry']." ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}
			$qry11 = mysql_query("SELECT * FROM tbl_users where status!=2 and usertype=1 ".$_SESSION['srchqry']."")or die(mysql_error());
		}
		else
		{
			if($_POST['fieldname'] == '0')
			{
				$qry = "SELECT * FROM tbl_users where status!=2 and usertype=1 order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_users where status!=2 and usertype=1 ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}	  
			$qry11 = mysql_query("SELECT * FROM tbl_users where status!=2 and usertype=1 ")or die(mysql_error());
		}
	...
	$(function(){
		...
		$('#form_artistsview input[type=checkbox]').checkBox('toggle');
		return false;
		});
	});
	</script> 
	<form id="form_artistsview" action="" name="form_artistsview" method="post" enctype="multipart/form-data" onsubmit="return false">
	<div class="searchdiv">
	...
	?>
		...
		<?php 
		 ...
		 ?>
			 <tr height="30">
			  <td colspan="7" align="center" style="color:#FF0000"><strong><?php echo "No artists found."; ?></strong></td>
			</tr>
		 <?php 
		 }
		 ?>
	...
	?>
	<script type="text/javascript" src="<?=$LOCATION['SITE_ADMIN']?>views/javascripts/js/jquery.form.js"></script><form name="form_artistsadd" id="form_artistsadd" method="post" enctype="multipart/form-data" action="#" >
		<table border="0" cellpadding="0" cellspacing="0"  id="id-form" width="100%"><tr class="white_bg">
					...
					?><tr class="white_bg">
						...
						<?php if($data['image']):?>
                        	<img src="<?=$_SESSION['SITE_NAME']?>upload/artist/thumb/<?php echo $data['image'] ?>" height="50" width="50" />
                        <?php endif;?>
