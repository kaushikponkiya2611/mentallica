<?php 
		...
		$upload_dir =$_SESSION['SITE_IMG_PATH']."sponser/";
		$upload_dirthumb =$_SESSION['SITE_IMG_PATH']."sponser/thumb/";
	
	if(isset($_POST['view']) && $_POST['view'] != ''){
		$id = $_POST['id'];
		$qry = "SELECT * FROM tbl_sponser where id = '".mysql_real_escape_string($id)."'";
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:13px"><tr class="white_bg">
						...
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border"><img src="<?=$_SESSION['SITE_NAME']?>upload/sponser/thumb/<?php echo stripslashes($result['image']) ?>" height="50" width="50" /></td>
					  </tr><tr class="light_bg">
						<td width="120" align="right" class="popup_listing_border"><strong>Link:</strong></td>
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border"><?php echo stripslashes($result['link']) ?></td>
					  </tr></table>
	<?php 
	} 
	?>			
	...
	if(isset($_POST['statusactive']) && $_POST['statusactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_sponser SET status = 1 WHERE id = ".mysql_real_escape_string($val);
	...
	if(isset($_POST['statusinactive']) && $_POST['statusinactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_sponser SET status = 0 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_sponser SET status = 2 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST['hid_add']) && $_POST['hid_add'] != ''){
		$qry = "SELECT * FROM tbl_sponser WHERE title = '".mysql_real_escape_string(trim($_POST['txt_addtitle']))."' and status!=2 ";
		$result = $modelObj -> fetchRow($qry);
		list($width, $height, $type, $attr) = getimagesize($_FILES["txt_addimage"]['tmp_name']);
		if(strtolower(trim($result['title'])) == strtolower(trim($_POST['txt_addtitle'])))
		{
			$flag = 0;
		}else if($width>400 && $height>160){
                   $flag = 3;
                }
		else
		{
			...
			if(!dir($upload_dirthumb))
			{
				mkdir($upload_dirthumb);
                        ...
                        if(isset($_FILES["txt_addimage"]["tmp_name"]))
                        {
                         ...
                         $qry = "INSERT INTO tbl_sponser (title,image,link,cr_date,status) VALUES ('".clear_input($_POST['txt_addtitle'])."','".$insertimage."','".clear_input($_POST['txt_addlink'])."',NOW(),1)";
			$result = $modelObj->runQuery($qry);
			echo "1";
           }else if($flag==3){
               echo "3";
	...
	if(isset($_POST['statusid']) && $_POST['statusid'] != ''){
		$qry = "UPDATE  tbl_sponser SET status = ".mysql_real_escape_string($_POST['status'])." WHERE id = ".mysql_real_escape_string($_POST['statusid']);
		$result =  $modelObj->runQuery($qry);
	}
	?>
	<?php
	if(isset($_POST['hid_update']) && $_POST['hid_update'] != ''){
		$qry = "SELECT * FROM tbl_sponser WHERE title = '".mysql_real_escape_string(trim($_POST['txt_addtitle']))."' and status!=2 and id != '".$_POST['hid_userid']."'";
		$result = $modelObj -> fetchRow($qry);
		...
		{if(isset($_FILES["txt_addimage"]["tmp_name"]))
				{
					...
					if($_FILES["txt_addimage"]["tmp_name"] != '')
					{
						...
						$qry_img="UPDATE  tbl_sponser SET image='".$imgname."' where id='".mysql_real_escape_string($_POST['hid_userid'])."' ";
						$res_img= $modelObj->runQuery($qry_img);
					}
				}
				 $qry = "UPDATE tbl_sponser SET title = '".clear_input($_POST['txt_addtitle'])."',link = '".clear_input($_POST['txt_addlink'])."' WHERE id = '".mysql_real_escape_string($_POST['hid_userid'])."'"; 
			$result = $modelObj->runQuery($qry);
			echo "1";
		}
		else
	...
	if(isset($_POST['viewdiv']) && $_POST['viewdiv'] != ''){
		...
		{if(trim($_POST['txt_srctitle']) != '')
			...
			{
				$qry = "SELECT * FROM tbl_sponser where status!=2 ".$_SESSION['srchqry']." order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_sponser where status!=2 ".$_SESSION['srchqry']." ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}
			$qry11 = mysql_query("SELECT * FROM tbl_sponser where status!=2 ".$_SESSION['srchqry']."")or die(mysql_error());
		}
		else
		{
			if($_POST['fieldname'] == '0')
			{
				$qry = "SELECT * FROM tbl_sponser where status!=2 order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_sponser where status!=2 ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}	  
			$qry11 = mysql_query("SELECT * FROM tbl_sponser where status!=2 ")or die(mysql_error());
		}
	...
	$(function(){
		...
		$('#form_sponserview input[type=checkbox]').checkBox('toggle');
		return false;
		});
	});
	</script> 
	<form id="form_sponserview" action="" name="form_sponserview" method="post" enctype="multipart/form-data" onsubmit="return false">
	<div class="searchdiv">
	...
	?>
		...
		<?php 
		 ...
		 ?>
			 <tr height="30">
			  <td colspan="7" align="center" style="color:#FF0000"><strong><?php echo "No advertisement found."; ?></strong></td>
			</tr>
		 <?php 
		 }
		 ?>
	...
	if(isset($_POST['delete']) && $_POST['delete'] != ''){
			$qry = "UPDATE tbl_sponser SET status = 2 WHERE id = '".$_POST['id']."'";
			$result = $modelObj->runQuery($qry);
		  if($result){
			echo $successmsg = '1';
		  }else{
	...
	if(isset($_POST['edit']) && $_POST['edit'] != ''){
		$qry = "SELECT * FROM tbl_sponser WHERE id = '".$_POST['id']."'";
	...
	?>
	<script type="text/javascript" src="<?=$LOCATION['SITE_ADMIN']?>views/javascripts/js/jquery.form.js"></script><form name="form_sponseradd" id="form_sponseradd" method="post" enctype="multipart/form-data" action="#" >
		<table border="0" cellpadding="0" cellspacing="0"  id="id-form" width="100%"><tr class="white_bg">
						...
						<img src="<?=$_SESSION['SITE_NAME']?>upload/sponser/thumb/<?php echo $data['image'] ?>" height="50" width="50" />
						<label class="removerror error_new" id="error-innertxt_addimage"></label>
						</td>
					</tr><tr class="white_bg">
						<th>Link : </th>
						<td width="23%">
						<input class="input_box" type="text" name="txt_addlink" id="txt_addlink" value="<?php echo stripslashes($data['link']) ?>" onblur="checkfield(this)"><font class="required"> *</font>
						</td>
						<td id="errortxt_addlink" width="57%">
						<label class="removerror error_new" id="error-innertxt_addlink"></label>
						</td>
					</tr><tr class="white_bg">
				<th>&nbsp;</th>
				<td valign="top">
