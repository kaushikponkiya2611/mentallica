<?php 
	...
	if(isset($_POST['view']) && $_POST['view'] != ''){
		$id = $_POST['id'];
		$qry = "SELECT * FROM tbl_customer where id = '".mysql_real_escape_string($id)."'";
	...
	if(isset($_POST['statusactive']) && $_POST['statusactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_customer SET status = 1 WHERE id = ".mysql_real_escape_string($val);
	...
	if(isset($_POST['statusinactive']) && $_POST['statusinactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_customer SET status = 0 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_customer SET status = 2 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST['hid_add']) && $_POST['hid_add'] != ''){
		$qry = "SELECT * FROM tbl_customer WHERE username = '".mysql_real_escape_string(trim($_POST['txt_addusername']))."' and status!=2 ";
		$result = $modelObj -> fetchRow($qry);
		
		if(strtolower(trim($result['username'])) == strtolower(trim($_POST['txt_addusername'])))
		{
			$flag = 0;
		}
		else
		...
		if($flag == 1)
		{
			...
			$code="US".rand(10,99).rand(10,99).rand(10,99);
			$qry = "INSERT INTO tbl_customer (ref_id,username,emailid,password,country,state,city,mobileno,cr_date,status) VALUES ('".clear_input($code)."','".clear_input($_POST['txt_addusername'])."','".clear_input($_POST['txt_addemailid'])."','".mysql_real_escape_string(trim(md5($_POST['txt_addpassword'])))."','".clear_input($_POST['txt_addcountry'])."','".clear_input($_POST['txt_addstate'])."','".clear_input($_POST['txt_addcity'])."','".clear_input($_POST['txt_addmobileno'])."',NOW(),1)";
			$result = $modelObj->runQuery($qry);
			echo "1";
	   }
	   else
	...
	if(isset($_POST['statusid']) && $_POST['statusid'] != ''){
		$qry = "UPDATE  tbl_customer SET status = ".mysql_real_escape_string($_POST['status'])." WHERE id = ".mysql_real_escape_string($_POST['statusid']);
		$result =  $modelObj->runQuery($qry);
	}
	?>
	<?php
	if(isset($_POST['hid_update']) && $_POST['hid_update'] != ''){
		$qry = "SELECT * FROM tbl_customer WHERE username = '".mysql_real_escape_string(trim($_POST['txt_addusername']))."' and status!=2 and id != '".$_POST['hid_userid']."'";
		$result = $modelObj -> fetchRow($qry);
		...
		if($flag == 1)
		{$qry = "UPDATE tbl_customer SET username = '".clear_input($_POST['txt_addusername'])."',emailid = '".clear_input($_POST['txt_addemailid'])."',country = '".clear_input($_POST['txt_addcountry'])."',state = '".clear_input($_POST['txt_addstate'])."',city = '".clear_input($_POST['txt_addcity'])."',mobileno = '".clear_input($_POST['txt_addmobileno'])."'WHERE id = '".mysql_real_escape_string($_POST['hid_userid'])."'";
			$result = $modelObj->runQuery($qry);
			echo "1";
		}
		else
	...
	if(isset($_POST['viewdiv']) && $_POST['viewdiv'] != ''){
		...
		{if(trim($_POST['txt_srcusername']) != '')
			...
			{
				$qry = "SELECT * FROM tbl_customer where status!=2 ".$_SESSION['srchqry']." order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_customer where status!=2 ".$_SESSION['srchqry']." ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}
			$qry11 = mysql_query("SELECT * FROM tbl_customer where status!=2 ".$_SESSION['srchqry']."")or die(mysql_error());
		}
		else
		{
			if($_POST['fieldname'] == '0')
			{
				$qry = "SELECT * FROM tbl_customer where status!=2 order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_customer where status!=2 ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}	  
			$qry11 = mysql_query("SELECT * FROM tbl_customer where status!=2 ")or die(mysql_error());
		}
	...
	$(function(){
		...
		$('#form_customerview input[type=checkbox]').checkBox('toggle');
		return false;
		});
	});
	</script> 
	<form id="form_customerview" action="" name="form_customerview" method="post" enctype="multipart/form-data" onsubmit="return false">
	<div class="searchdiv">
	...
	?>
		<tr id="<?php echo $data['id']?>" class="<?php if($i%2==0){ echo "light_bg"; }else{ echo "white_bg"; } ?>" height="30">
			<td><input  type="checkbox" name="chk_id" id="chk_id" value="<?php echo $data['id'];  ?>"/></td><td class="cursorpointer" onclick="edit('<?php echo $data['id'] ?>','<?php echo $_SESSION['pid']?>')"><?php echo stripslashes($data['username']); ?></td><td><?php echo stripslashes($data['emailid']); ?></td><td><?php echo stripslashes($data['mobileno']); ?></td><td class="options-width" >
				<table>
					<tr>
						<td>
							<?php
		...
		<?php 
		 ...
		 ?>
			 <tr height="30">
			  <td colspan="7" align="center" style="color:#FF0000"><strong><?php echo "No customer found."; ?></strong></td>
			</tr>
		 <?php 
		 }
		 ?>
	...
	if(isset($_POST['delete']) && $_POST['delete'] != ''){
			$qry = "UPDATE tbl_customer SET status = 2 WHERE id = '".$_POST['id']."'";
			$result = $modelObj->runQuery($qry);
		  if($result){
			echo $successmsg = '1';
		  }else{
	...
	if(isset($_POST['edit']) && $_POST['edit'] != ''){
		$qry = "SELECT * FROM tbl_customer WHERE id = '".$_POST['id']."'";
	...
	?>
	<script type="text/javascript" src="<?=$LOCATION['SITE_ADMIN']?>views/javascripts/js/jquery.form.js"></script><form name="form_customeradd" id="form_customeradd" method="post" enctype="multipart/form-data" action="#" >
		<table border="0" cellpadding="0" cellspacing="0"  id="id-form" width="100%"><tr class="white_bg">
						<th>Username : </th>
						<td width="23%">
						<input class="input_box" type="text" name="txt_addusername" id="txt_addusername" value="<?php echo stripslashes($data['username']) ?>" onblur="checkfield(this)">
						</td>
					...
					<td id="cityajax" width="23%">
						...
						<th>Mobileno : </th>
						<td width="23%">
						<input class="input_box" type="text" name="txt_addmobileno" id="txt_addmobileno" value="<?php echo stripslashes($data['mobileno']) ?>" onblur="checkfield(this)">
						</td>
