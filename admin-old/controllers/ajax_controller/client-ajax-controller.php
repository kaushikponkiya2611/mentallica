<?php 
	...
	if(isset($_POST['view']) && $_POST['view'] != ''){
		$id = $_POST['id'];
		$qry = "SELECT * FROM tbl_users where id = '".mysql_real_escape_string($id)."' and usertype=2";
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:13px"><tr class="white_bg">
					...
					<tr class="light_bg">
						...
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border"><?php if($result['image']):?>
						...
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border">Client</td>
					  </tr></table>
	<?php 
	} 
	?>			
	...
	if(isset($_POST['viewdiv']) && $_POST['viewdiv'] != ''){
		...
		{if(trim($_POST['txt_srcref_id']) != '')
			...
			{
				$qry = "SELECT * FROM tbl_users where status!=2 and usertype=2 ".$_SESSION['srchqry']." order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_users where status!=2 and usertype=2 ".$_SESSION['srchqry']." ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}
			$qry11 = mysql_query("SELECT * FROM tbl_users where status!=2 and usertype=2 ".$_SESSION['srchqry']."")or die(mysql_error());
		}
		else
		{
			if($_POST['fieldname'] == '0')
			{
				$qry = "SELECT * FROM tbl_users where status!=2 and usertype=2 order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_users where status!=2 and usertype=2 ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}	  
			$qry11 = mysql_query("SELECT * FROM tbl_users where status!=2 and usertype=2 ")or die(mysql_error());
		}
	...
	<form id="form_clientview" action="" name="form_clientview" method="post" enctype="multipart/form-data" onsubmit="return false">
	<div class="searchdiv">
	...
	?>
	<script type="text/javascript" src="<?=$LOCATION['SITE_ADMIN']?>views/javascripts/js/jquery.form.js"></script><form name="form_clientadd" id="form_clientadd" method="post" enctype="multipart/form-data" action="#" >
