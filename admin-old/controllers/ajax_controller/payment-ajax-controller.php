<?php 
	...
	if(isset($_POST['view']) && $_POST['view'] != ''){
		$id = $_POST['id'];
		$qry = "SELECT * FROM tbl_payment where id = '".mysql_real_escape_string($id)."'";
		...
		$qry = "SELECT first_name, last_name, image_limit FROM tbl_users where status!=2 AND id='".clear_input($result['user_id'])."' LIMIT 1";
		  	...
		  	$qry = "SELECT plan_name FROM tbl_plans where status!=2 AND id='".clear_input($result['plan_id'])."' LIMIT 1";
		  	$planname = $modelObj->fetchRow($qry);
	?>
	<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:13px"><tr class="white_bg">
						<td width="120" align="right" class="popup_listing_border"><strong>User Name:</strong></td>
						...
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border"><?php echo stripslashes($planname['plan_name']) ?></td>
					  </tr><tr class="white_bg">
						<td width="120" align="right" class="popup_listing_border"><strong>Transaction Id:</strong></td>
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border"><?php echo stripslashes($result['transaction_id']) ?></td>
					  </tr><tr class="light_bg">
						<td width="120" align="right" class="popup_listing_border"><strong>Amount:</strong></td>
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border"><?php echo stripslashes($result['amount']) ?></td>
					  </tr></table>
	<?php 
	} 
	?>			
	...
	if(isset($_POST['statusactive']) && $_POST['statusactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_payment SET status = 1 WHERE id = ".mysql_real_escape_string($val);
	...
	if(isset($_POST['statusinactive']) && $_POST['statusinactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_payment SET status = 0 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_payment SET status = 2 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST['hid_add']) && $_POST['hid_add'] != ''){
		$qry = "SELECT * FROM tbl_payment WHERE user_id = '".mysql_real_escape_string(trim($_POST['txt_adduser_id']))."' and status!=2 ";
		$result = $modelObj -> fetchRow($qry);
		...
		if($flag == 1)
		{
			...
			}$qry = "INSERT INTO tbl_payment (user_id,plan_id,transaction_id,amount,cr_date,status) VALUES ('".clear_input($_POST['txt_adduser_id'])."','".clear_input($_POST['txt_addplan_id'])."','".clear_input($_POST['txt_addtransaction_id'])."','".clear_input($_POST['txt_addamount'])."',NOW(),1)";
			$result = $modelObj->runQuery($qry);
			echo "1";
	   }
	   else
	...
	if(isset($_POST['statusid']) && $_POST['statusid'] != ''){
		$qry = "UPDATE  tbl_payment SET status = ".mysql_real_escape_string($_POST['status'])." WHERE id = ".mysql_real_escape_string($_POST['statusid']);
		$result =  $modelObj->runQuery($qry);
	}
	?>
	<?php
	if(isset($_POST['hid_update']) && $_POST['hid_update'] != ''){
		$qry = "SELECT * FROM tbl_payment WHERE user_id = '".mysql_real_escape_string(trim($_POST['txt_adduser_id']))."' and status!=2 and id != '".$_POST['hid_userid']."'";
		$result = $modelObj -> fetchRow($qry);
		...
		if($flag == 1)
		{$qry = "UPDATE tbl_payment SET user_id = '".clear_input($_POST['txt_adduser_id'])."',plan_id = '".clear_input($_POST['txt_addplan_id'])."',transaction_id = '".clear_input($_POST['txt_addtransaction_id'])."',amount = '".clear_input($_POST['txt_addamount'])."'WHERE id = '".mysql_real_escape_string($_POST['hid_userid'])."'";
			$result = $modelObj->runQuery($qry);
			echo "1";
		}
		else
	...
	if(isset($_POST['viewdiv']) && $_POST['viewdiv'] != ''){
		...
		{if(trim($_POST['txt_srcuser_id']) != '')
						...
						}if(trim($_POST['txt_srcplan_id']) != '')
						{
							...
							$_SESSION['srchqry'] =  $searchqry; 
						}if(trim($_POST['txt_srctransaction_id']) != '')
						{
							$searchqry .="and transaction_id LIKE '%".mysql_real_escape_string(trim($_POST['txt_srctransaction_id']))."%'";
							$_SESSION['srchqry'] =  $searchqry; 
						}if(trim($_POST['txt_srcamount']) != '')
						{
							$searchqry .="and amount LIKE '%".mysql_real_escape_string(trim($_POST['txt_srcamount']))."%'";
							$_SESSION['srchqry'] =  $searchqry; 
						}$_SESSION['srchqry'] =  $_SESSION['srchqry']; 
			
			if($_POST['fieldname'] == '0')
			{
				$qry = "SELECT * FROM tbl_payment where status!=2 ".$_SESSION['srchqry']." order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_payment where status!=2 ".$_SESSION['srchqry']." ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}
			$qry11 = mysql_query("SELECT * FROM tbl_payment where status!=2 ".$_SESSION['srchqry']."")or die(mysql_error());
		}
		else
		{
			if($_POST['fieldname'] == '0')
			{
				$qry = "SELECT * FROM tbl_payment where status!=2 order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_payment where status!=2 ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}	  
			$qry11 = mysql_query("SELECT * FROM tbl_payment where status!=2 ")or die(mysql_error());
		}
	...
	$(function(){
		...
		$('#form_paymentview input[type=checkbox]').checkBox('toggle');
		return false;
		});
	});
	</script> 
	<form id="form_paymentview" action="" name="form_paymentview" method="post" enctype="multipart/form-data" onsubmit="return false">
	<div class="searchdiv">
	...
	<table border="0" width="100%" cellpadding="0" cellspacing="0" id="product-table">
		<tr>
		  <th class="table-header-check"><a id="toggle-all" ></a> </th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('user_id','<?php if($orderby=='asc'){echo "desc";}else{echo "asc";} ?>')" id="user_id" class="cursorpointer">User Name</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('plan_id','<?php if($orderby=='asc'){echo "desc";}else{echo "asc";} ?>')" id="plan_id" class="cursorpointer">Plan Name</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('transaction_id','<?php if($orderby=='asc'){echo "desc";}else{echo "asc";} ?>')" id="transaction_id" class="cursorpointer">Transaction Id</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('amount','<?php if($orderby=='asc'){echo "desc";}else{echo "asc";} ?>')" id="amount" class="cursorpointer">Amount</a></th><th class="table-header-options line-left"><a>Options</a></th>
		</tr>
	<?php
	$i = 0;
	if($result != ''){
		foreach($result  as $k => $data){
			$qry = "SELECT first_name, last_name, image_limit FROM tbl_users where status!=2 AND id='".clear_input($data['user_id'])."' LIMIT 1";
		  	$username = $modelObj->fetchRow($qry);
		  	$qry = "SELECT plan_name FROM tbl_plans where status!=2 AND id='".clear_input($data['plan_id'])."' LIMIT 1";
		  	$planname = $modelObj->fetchRow($qry);
		$i++;
	?>
		<tr id="<?php echo $data['id']?>" class="<?php if($i%2==0){ echo "light_bg"; }else{ echo "white_bg"; } ?>" height="30">
			<td><input  type="checkbox" name="chk_id" id="chk_id" value="<?php echo $data['id'];  ?>"/></td><td class="cursorpointer" onclick="edit('<?php echo $data['id'] ?>','<?php echo $_SESSION['pid']?>')"><?php echo stripslashes($username['first_name']." ".$username['last_name']); ?></td><td><?php echo stripslashes($planname['plan_name']); ?></td><td><?php echo stripslashes($data['transaction_id']); ?></td><td><?php echo stripslashes($data['amount']); ?></td><td class="options-width" >
				<table>
					<tr>
						<!--<td>
							<?php
		...
		<?php 
		 ...
		 ?>
			 <tr height="30">
			  <td colspan="7" align="center" style="color:#FF0000"><strong><?php echo "No payment found."; ?></strong></td>
			</tr>
		 <?php 
		 }
		 ?>
	...
	if(isset($_POST['delete']) && $_POST['delete'] != ''){
			$qry = "UPDATE tbl_payment SET status = 2 WHERE id = '".$_POST['id']."'";
			$result = $modelObj->runQuery($qry);
		  if($result){
			echo $successmsg = '1';
		  }else{
	...
	if(isset($_POST['edit']) && $_POST['edit'] != ''){
		$qry = "SELECT * FROM tbl_payment WHERE id = '".$_POST['id']."'";
	...
	?>
	<script type="text/javascript" src="<?=$LOCATION['SITE_ADMIN']?>views/javascripts/js/jquery.form.js"></script><form name="form_paymentadd" id="form_paymentadd" method="post" enctype="multipart/form-data" action="#" >
		<table border="0" cellpadding="0" cellspacing="0"  id="id-form" width="100%"><tr class="white_bg">
						<th>User Id : </th>
						<td width="23%">
						<input class="input_box" type="text" name="txt_adduser_id" id="txt_adduser_id" value="<?php echo stripslashes($data['user_id']) ?>" onblur="checkfield(this)"><font class="required"> *</font>
						</td>
						...
						<td width="23%">
						<input class="input_box" type="text" name="txt_addtransaction_id" id="txt_addtransaction_id" value="<?php echo stripslashes($data['transaction_id']) ?>" onblur="checkfield(this)"><font class="required"> *</font>
						</td>
						<td id="errortxt_addtransaction_id" width="57%">
						<label class="removerror error_new" id="error-innertxt_addtransaction_id"></label>
						</td>
					</tr><tr class="light_bg">
						<th>Amount : </th>
						<td width="23%">
						<input class="input_box" type="text" name="txt_addamount" id="txt_addamount" value="<?php echo stripslashes($data['amount']) ?>" onblur="checkfield(this)"><font class="required"> *</font>
						</td>
						<td id="errortxt_addamount" width="57%">
						<label class="removerror error_new" id="error-innertxt_addamount"></label>
						</td>
					</tr><tr class="white_bg">
				<th>&nbsp;</th>
				<td valign="top">
