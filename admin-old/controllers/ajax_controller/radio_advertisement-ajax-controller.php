<?php 
		...
		$upload_dir =$_SESSION['SITE_IMG_PATH']."radio/";
		$upload_dirthumb =$_SESSION['SITE_IMG_PATH']."radio/thumb/";
	
	if(isset($_POST['view']) && $_POST['view'] != ''){
		$id = $_POST['id'];
		$qry = "SELECT * FROM tbl_radio_advertisement where id = '".mysql_real_escape_string($id)."'";
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:13px"><tr class="white_bg">
						...
						<td width="120" align="right" class="popup_listing_border"><strong>Time:</strong></td>
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border"><?php echo stripslashes($result['time']) ?></td>
					  </tr><tr class="light_bg">
						<td width="120" align="right" class="popup_listing_border"><strong>Start Time:</strong></td>
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border"><?php echo stripslashes($result['start_time']) ?></td>
					  </tr><tr class="white_bg">
						<td width="120" align="right" class="popup_listing_border"><strong>End Time:</strong></td>
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border"><?php echo stripslashes($result['end_time']) ?></td>
					  </tr><tr class="light_bg">
						<td width="120" align="right" class="popup_listing_border"><strong>Weight:</strong></td>
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border"><?php echo stripslashes($result['weight']) ?></td>
					  </tr></table>
	<?php 
	} 
	?>			
	...
	if(isset($_POST['statusactive']) && $_POST['statusactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_radio_advertisement SET status = 1 WHERE id = ".mysql_real_escape_string($val);
	...
	if(isset($_POST['statusinactive']) && $_POST['statusinactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_radio_advertisement SET status = 0 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_radio_advertisement SET status = 2 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST['hid_add']) && $_POST['hid_add'] != ''){
		$qry = "SELECT * FROM tbl_radio_advertisement WHERE title = '".mysql_real_escape_string(trim($_POST['txt_addtitle']))."' and status!=2 ";
		$result = $modelObj -> fetchRow($qry);
		...
		if($flag == 1)
		{
			...
			if(!dir($upload_dirthumb))
			{
				...
				}$qry = "INSERT INTO tbl_radio_advertisement (title,image,time,start_time,end_time,weight,cr_date,status) VALUES ('".clear_input($_POST['txt_addtitle'])."','".$insertimage."','".clear_input($_POST['txt_addtime'])."','".clear_input($_POST['txt_addstart_time'])."','".clear_input($_POST['txt_addend_time'])."','".clear_input($_POST['txt_addweight'])."',NOW(),1)";
			$result = $modelObj->runQuery($qry);
			echo "1";
	   }
	   else
	...
	if(isset($_POST['statusid']) && $_POST['statusid'] != ''){
		$qry = "UPDATE  tbl_radio_advertisement SET status = ".mysql_real_escape_string($_POST['status'])." WHERE id = ".mysql_real_escape_string($_POST['statusid']);
		$result =  $modelObj->runQuery($qry);
	}
	?>
	<?php
	if(isset($_POST['hid_update']) && $_POST['hid_update'] != ''){
		$qry = "SELECT * FROM tbl_radio_advertisement WHERE title = '".mysql_real_escape_string(trim($_POST['txt_addtitle']))."' and status!=2 and id != '".$_POST['hid_userid']."'";
		$result = $modelObj -> fetchRow($qry);
		...
		{if(isset($_FILES["txt_addimage"]["tmp_name"]))
				{
					...
					if($_FILES["txt_addimage"]["tmp_name"] != '')
					{
						...
						$qry_img="UPDATE  tbl_radio_advertisement SET image='".$imgname."' where id='".mysql_real_escape_string($_POST['hid_userid'])."' ";
						$res_img= $modelObj->runQuery($qry_img);
					}
				}$qry = "UPDATE tbl_radio_advertisement SET title = '".clear_input($_POST['txt_addtitle'])."',time = '".clear_input($_POST['txt_addtime'])."',start_time = '".clear_input($_POST['txt_addstart_time'])."',end_time = '".clear_input($_POST['txt_addend_time'])."',weight = '".clear_input($_POST['txt_addweight'])."'WHERE id = '".mysql_real_escape_string($_POST['hid_userid'])."'";
			$result = $modelObj->runQuery($qry);
			echo "1";
		}
		else
	...
	if(isset($_POST['viewdiv']) && $_POST['viewdiv'] != ''){
		...
		{if(trim($_POST['txt_srctitle']) != '')
						...
						}if(trim($_POST['txt_srctime']) != '')
						{
							$searchqry .="and time LIKE '%".mysql_real_escape_string(trim($_POST['txt_srctime']))."%'";
							$_SESSION['srchqry'] =  $searchqry; 
						}if(trim($_POST['txt_srcstart_time']) != '')
						{
							$searchqry .="and start_time LIKE '%".mysql_real_escape_string(trim($_POST['txt_srcstart_time']))."%'";
							$_SESSION['srchqry'] =  $searchqry; 
						}if(trim($_POST['txt_srcend_time']) != '')
						{
							$searchqry .="and end_time LIKE '%".mysql_real_escape_string(trim($_POST['txt_srcend_time']))."%'";
							$_SESSION['srchqry'] =  $searchqry; 
						}if(trim($_POST['txt_srcweight']) != '')
						{
							$searchqry .="and weight LIKE '%".mysql_real_escape_string(trim($_POST['txt_srcweight']))."%'";
							$_SESSION['srchqry'] =  $searchqry; 
						}$_SESSION['srchqry'] =  $_SESSION['srchqry']; 
			
			if($_POST['fieldname'] == '0')
			{
				$qry = "SELECT * FROM tbl_radio_advertisement where status!=2 ".$_SESSION['srchqry']." order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_radio_advertisement where status!=2 ".$_SESSION['srchqry']." ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}
			$qry11 = mysql_query("SELECT * FROM tbl_radio_advertisement where status!=2 ".$_SESSION['srchqry']."")or die(mysql_error());
		}
		else
		{
			if($_POST['fieldname'] == '0')
			{
				$qry = "SELECT * FROM tbl_radio_advertisement where status!=2 order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_radio_advertisement where status!=2 ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}	  
			$qry11 = mysql_query("SELECT * FROM tbl_radio_advertisement where status!=2 ")or die(mysql_error());
		}
	...
	$(function(){
		...
		$('#form_radio_advertisementview input[type=checkbox]').checkBox('toggle');
		return false;
		});
	});
	</script> 
	<form id="form_radio_advertisementview" action="" name="form_radio_advertisementview" method="post" enctype="multipart/form-data" onsubmit="return false">
	<div class="searchdiv">
	...
	<table border="0" width="100%" cellpadding="0" cellspacing="0" id="product-table">
		<tr>
		  <th class="table-header-check"><a id="toggle-all" ></a> </th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('title','<?php if($orderby=='asc'){echo "desc";}else{echo "asc";} ?>')" id="title" class="cursorpointer">Title</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('time','<?php if($orderby=='asc'){echo "desc";}else{echo "asc";} ?>')" id="time" class="cursorpointer">Time</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('start_time','<?php if($orderby=='asc'){echo "desc";}else{echo "asc";} ?>')" id="start_time" class="cursorpointer">Start Time</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('end_time','<?php if($orderby=='asc'){echo "desc";}else{echo "asc";} ?>')" id="end_time" class="cursorpointer">End Time</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('weight','<?php if($orderby=='asc'){echo "desc";}else{echo "asc";} ?>')" id="weight" class="cursorpointer">Weight</a></th><th class="table-header-options line-left"><a>Options</a></th>
		</tr>
	<?php
	$i = 0;
	if($result != ''){
	...
	?>
		...
		<?php 
		 ...
		 ?>
			 <tr height="30">
			  <td colspan="7" align="center" style="color:#FF0000"><strong><?php echo "No radio_advertisement found."; ?></strong></td>
			</tr>
		 <?php 
		 }
		 ?>
	...
	if(isset($_POST['delete']) && $_POST['delete'] != ''){
			$qry = "UPDATE tbl_radio_advertisement SET status = 2 WHERE id = '".$_POST['id']."'";
			$result = $modelObj->runQuery($qry);
		  if($result){
			echo $successmsg = '1';
		  }else{
	...
	if(isset($_POST['edit']) && $_POST['edit'] != ''){
		$qry = "SELECT * FROM tbl_radio_advertisement WHERE id = '".$_POST['id']."'";
	...
	?>
	<script type="text/javascript" src="<?=$LOCATION['SITE_ADMIN']?>views/javascripts/js/jquery.form.js"></script><form name="form_radio_advertisementadd" id="form_radio_advertisementadd" method="post" enctype="multipart/form-data" action="#" >
		<table border="0" cellpadding="0" cellspacing="0"  id="id-form" width="100%"><tr class="white_bg">
						...
						<th>Time : </th>
						<td width="23%">
						<input class="input_box" type="text" name="txt_addtime" id="txt_addtime" value="<?php echo stripslashes($data['time']) ?>" onblur="checkfield(this)"><font class="required"> *</font>
						</td>
						<td id="errortxt_addtime" width="57%">
						<label class="removerror error_new" id="error-innertxt_addtime"></label>
						</td>
					</tr><tr class="light_bg">
						<th>Start Time : </th>
						<td width="23%">
						<input class="input_box" type="text" name="txt_addstart_time" id="txt_addstart_time" value="<?php echo stripslashes($data['start_time']) ?>" onblur="checkfield(this)"><font class="required"> *</font>
						</td>
						<td id="errortxt_addstart_time" width="57%">
						<label class="removerror error_new" id="error-innertxt_addstart_time"></label>
						</td>
					</tr><tr class="white_bg">
						<th>End Time : </th>
						<td width="23%">
						<input class="input_box" type="text" name="txt_addend_time" id="txt_addend_time" value="<?php echo stripslashes($data['end_time']) ?>" onblur="checkfield(this)"><font class="required"> *</font>
						</td>
						<td id="errortxt_addend_time" width="57%">
						<label class="removerror error_new" id="error-innertxt_addend_time"></label>
						</td>
					</tr><tr class="light_bg">
						<th>Weight : </th>
						<td width="23%">
						<input class="input_box" type="text" name="txt_addweight" id="txt_addweight" value="<?php echo stripslashes($data['weight']) ?>" onblur="checkfield(this)"><font class="required"> *</font>
						</td>
						<td id="errortxt_addweight" width="57%">
						<label class="removerror error_new" id="error-innertxt_addweight"></label>
						</td>
					</tr><tr class="white_bg">
				<th>&nbsp;</th>
				<td valign="top">
