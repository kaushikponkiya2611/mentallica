function forgot_password_function(){
    ...
    if($("#txt_forgot_email").val() == ''){
        ...
        $("#forg_txt_forgot_emailemail").css("border-color","red");
        return false;
    }else if(emailchk.test($("#txt_forgot_email").val())==false){
        $("#txt_forgot_email").focus();
        $("#txt_forgot_email").css("border-color","red");
        return false;
    }
    ...
    $.ajax( {
        ...
        success : function(result){
            ...
            if(result == 1){
                ...
                $("#forg_content").html('Email adress does not exist please try again.');
                $("#forg_title").html("Successful");
                $('#various_1').fancybox().trigger('click');
                return false;
            } else if(result == 2){
                $("#forg_content").html('Entered email address is not exist in database.');
                $("#forg_title").html("Validate");
                $('#various_1').fancybox().trigger('click');
                return false;
            }
        }
...
function sendmessagefunction(){
    ...
    if($("#txt_sendmsgto").val() == ''){
        $("#error-innertxt_sendmsgto").show().fadeOut(5000);
        $("#error-innertxt_sendmsgto").html('This field is required');
        $("#txt_sendmsgto").focus();
        return false;
    }else if(emailchk.test($("#txt_sendmsgto").val()) == false){
        $("#error-innertxt_sendmsgto").show().fadeOut(5000);
        $("#error-innertxt_sendmsgto").html('Invalid email address');
        $("#txt_sendmsgto").focus();
        return false;
    }
    else{
        //alert("IN")
        var options = {
            beforeSubmit:  showRequestSendMessage,
            success:       showResponseSendMessage,
            url:       site_url+'controllers/ajax_controller/comman-ajax-controller.php', 
            type: "POST"
        };
        $('#form_sendmessage').submit(function() {
            $(this).ajaxSubmit(options);
            return false;
        });
    }
}
function showRequestSendMessage(formData, jqForm, options) {
	return true;
}
function showResponseSendMessage(data, statusText)  {
	if(statusText == 'success'){
            if(data == 0){
                $.scrollTo(0,500);
                $("#message-red").show().fadeOut(7000);
                $("#message-green").hide();
                document.getElementById('err').innerHTML = 'Some error occured. Please try another.';
            }else if(data == 1){
                $("#message-red").hide();
                $("#message-green").show().fadeOut(5000);		   
                document.getElementById('succ').innerHTML = 'Email sent successfully.';
                document.getElementById("form_sendmessage").reset();		 
            }
            else if(data == 2){
                $("#message-red").hide();
                $("#message-green").show().fadeOut(5000);		   
                document.getElementById('succ').innerHTML = 'Email address does not exist in databse please try again.';
                document.getElementById("form_sendmessage").reset();		 
            }
            $('#form_sendmessage').unbind('submit').bind('submit',function() {});
	}
}
function sendmessageemployers(){
    var alpha = /^[a-zA-Z]+$/;
    var alphanum = /^[a-zA-Z0-9]+$/;
    var emailchk = /^[\w\-\.\+]+\@[a-zA-Z0-9\.\-]+\.[a-zA-Z0-9]{2,4}$/;
    var mobnum=/^[0-9]{10,12}$/;
    ...
    var options = {
        beforeSubmit:showRequestSendMessageepmer,
        success:showResponseSendMessagetestepmer,
        url:site_url+'controllers/ajax_controller/comman-ajax-controller.php', 
        type: "POST"
    };
    $('#form_sendmessage').submit(function() {
...
function showRequestSendMessageepmer(formData, jqForm, options) {
    return true;
}
function showResponseSendMessagetestepmer(data, statusText)  {
    if(statusText == 'success'){
        if(data == 0){
            $.scrollTo(0,500);
            $("#message-red").show().fadeOut(7000);
...
function sendmessagejobseekers(){
	var alpha = /^[a-zA-Z]+$/;
	var alphanum = /^[a-zA-Z0-9]+$/;
	var emailchk = /^[\w\-\.\+]+\@[a-zA-Z0-9\.\-]+\.[a-zA-Z0-9]{2,4}$/;
	var mobnum=/^[0-9]{10,12}$/;
	...
	if($("#txt_sendmsgsub").val() == ''){
	  $("#error-innertxt_sendmsgsub").show().fadeOut(5000);
	  $("#error-innertxt_sendmsgsub").html('This field is required');
	  $("#txt_sendmsgsub").focus();
	  return false;
	}else if(emailchk.test($("#txt_sendmsgto").val()) == false){
	  $("#error-innertxt_sendmsgto").show().fadeOut(5000);
	  $("#error-innertxt_sendmsgto").html('Invalid email address');
	  ...
	  return false;
            ...
            var options = {
                beforeSubmit:  showRequestSendMessageseeker,
                success:       showResponseSendMessageseeker,
                url:       site_url+'controllers/ajax_controller/comman-ajax-controller.php', 
                type: "POST"
            };
            $('#form_sendmessage').submit(function() {
...
function showRequestSendMessageseeker(formData, jqForm, options) {
	return true;
}
function showResponseSendMessageseeker(data, statusText)  {
	//alert(data);
    if(statusText == 'success'){
        if(data == 0){
            $.scrollTo(0,500);
...
function deleteuser(id){
	...
	$("#h_delete").val(id);
	$("#delete_line").html('Are you sure to delete this '+pid_lower+'?');
