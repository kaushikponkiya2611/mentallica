function searching(){
	...
	else
	{
		if(regflag == 0)
		{
			...
			var options = {
				...
				url:       site_url+'controllers/ajax_controller/radio-ajax-controller.php', 
				type: "POST"
			};
	...
	if(statusText == 'success')
	{
		if(data == 0)
		{
			...
			$("#message-green").hide();
			document.getElementById('err').innerHTML = 'Radio already exist. Please try another.';
		}else if(data == 1){
			$("#message-red").hide();
			$("#message-green").show().fadeOut(5000);		   
			document.getElementById('succ').innerHTML = 'Radio added successfully.';
			newdata();				 
		}else if(data == 2){
			$.scrollTo(0,500);
			$("#message-red").show().fadeOut(7000);
			$("#message-green").hide();
			document.getElementById('err').innerHTML = 'Some error occurred while adding radio.';
		}
		$('#form_radioadd').unbind('submit').bind('submit',function() {
		});
	}
	}
	function updatedata(){
	...
	function showResponse_update(data, statusText)  
	{
		if (statusText == 'success') 
		{
			if(data == 0){
				...
				$("#message-red").hide();
				$("#message-green").show().fadeOut(5000);				
				document.getElementById('succ').innerHTML = 'Radio updated successfully.';
				newdata();
			}else if(data == 2){
				$.scrollTo(0,500);
				$("#message-red").show().fadeOut(7000);
				$("#message-green").hide();
				document.getElementById('err').innerHTML = 'Some error occurred while updating radio.';
			}
		}
		$('#form_radioadd').unbind('submit').bind('submit',function() {
		});
