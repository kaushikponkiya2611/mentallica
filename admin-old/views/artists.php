<script type="text/javascript">
	...
	<script type="text/javascript" src="<?=$LOCATION['SITE_ADMIN']?>views/javascripts/artistsscripts.js"></script>
	  ...
	  <table width="650" border="0" align="center" cellpadding="0" cellspacing="0">
		  <tr>
			<td width="635" height="35" class="popup_bg" ><table width="620" border="0" cellspacing="0" cellpadding="0" align="left">
				<tr>
				  <td colspan="2" >Upload Artists CSV</td>
				</tr>
			  </table></td>
		  </tr>
		  <tr>
			<td height="20" align="right" class="popup_listing_border"><a href="<?=$LOCATION['SITE_ADMIN']?>csv-examples/artists.csv">Example of CSV Format</a></td>
		  </tr>
		  <tr>
	  ...
	  <div id="<?php echo $_GET['pid'] ?>" >
		<div class="searchdiv">
		  <table class="searchdiv" border="0" width="100%" cellpadding="0" cellspacing="0">
			...
			if($controller_class -> getartists != ''){
			?>
		...
		<form id="form_artistsview" action="" name="form_artistsview" method="post" enctype="multipart/form-data" >
		  <table border="0" width="100%" cellpadding="0" cellspacing="0" id="product-table">
			<tr>
			  <th class="table-header-check"><a id="toggle-all" ></a> </th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('ref_id','asc')" id="ref_id" class="cursorpointer">Ref Id</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('first_name','asc')" id="first_name" class="cursorpointer">First Name</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('last_name','asc')" id="last_name" class="cursorpointer">Last Name</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('emailid','asc')" id="emailid" class="cursorpointer">Emailid</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('mobileno','asc')" id="mobileno" class="cursorpointer">Mobileno</a></th><th class="table-header-options line-left"><a>Options</a></th>
			</tr>
			<?php
	   $i = 0;
		if($controller_class -> getartists != ''){
			foreach($controller_class -> getartists  as $k => $data){
				$i++;
	  ?>
			<tr id="<?php echo $data['id']?>" class="<?php if($i%2==0){ echo "light_bg"; }else{ echo "white_bg"; } ?>" height="30">
			  <td><input  type="checkbox" name="chk_id" id="chk_id" value="<?php echo $data['id'];  ?>"/></td><td class="cursorpointer" onclick="edit('<?php echo $data['id'] ?>','<?php echo $_GET['pid']?>')"><?php echo $data['ref_id']; ?></td><td><?php echo $data['first_name']; ?></td><td><?php echo $data['last_name']; ?></td><td><?php echo $data['emailid']; ?></td><td><?php echo $data['mobileno']; ?></td><td class="options-width" ><table>
				  <tr>
					<td><?php
							if($data['status'] == '1')
							{
