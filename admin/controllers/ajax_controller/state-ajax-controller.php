...
if(isset($_POST['view']) && $_POST['view'] != ''){
    $id = $_POST['id'];
	$qry = "SELECT * FROM regions WHERE id = '".mysql_real_escape_string($id)."'";
	$result = $modelObj->fetchRow($qry);
	$qry_state="SELECT * FROM countries WHERE id='".$result["country_id"]."' ";
	$result_state= $modelObj->fetchRow($qry_state);
?>
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:13px">
      <tr class="light_bg">
        <td width="120" align="right" class="popup_listing_border"><strong>State Name :</strong></td>
...
if(isset($_POST['statusactive']) && $_POST['statusactive'] != '')
{
	...
	foreach($id as $k => $val)
	{
		$qry = "UPDATE  regions  SET status = 1 WHERE id = ".mysql_real_escape_string($val);
...
if(isset($_POST['statusinactive']) && $_POST['statusinactive'] != '')
{
	...
	foreach($id as $k => $val)
	{
		$qry = "UPDATE  regions  SET status = 0 WHERE id = ".mysql_real_escape_string($val);
		$result =  $modelObj->runQuery($qry);
	}
}
?>
...
if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
	...
	foreach($id as $k => $val)
	{
		$qry = "UPDATE  regions  SET status = 2 WHERE id = ".mysql_real_escape_string($val);
		$result =  $modelObj->runQuery($qry);
		$qry_city = "UPDATE  tbl_city  SET status = 2 WHERE stateid = ".mysql_real_escape_string($val);
		$result =  $modelObj->runQuery($qry_city);
		$qry_zip = "UPDATE  tbl_zipcode  SET status = 2 WHERE stateid = ".mysql_real_escape_string($val);
...
if(isset($_POST['hid_add']) && $_POST['hid_add'] != ''){
	$qry = "SELECT name FROM regions WHERE status!=2 and name = '".mysql_real_escape_string(trim($_POST['txt_state']))."'";
	$result = $modelObj -> fetchRow($qry);
	
	if(strtolower(trim($result['name'])) == strtolower(trim($_POST['txt_state'])))
	{
		$flag = 0;
	}
	else
	...
	if($flag == 1)
	{
		$qry = "INSERT INTO regions (name,country_id,status) 
		VALUES ('".clear_input($_POST['txt_state'])."',
		'".clear_input($_POST['txt_countrya'])."',1)";
		$result = $modelObj->runQuery($qry);
		if($result)
		{
			echo "1";
...
if(isset($_POST['statusid']) && $_POST['statusid'] != ''){
	$qry = "UPDATE  regions  SET status = ".mysql_real_escape_string($_POST['status'])." WHERE id = ".mysql_real_escape_string($_POST['statusid']);
	$result =  $modelObj->runQuery($qry);
}
?>
<?php
if(isset($_POST['hid_update']) && $_POST['hid_update'] != ''){
	$qry = "SELECT name FROM regions WHERE status!=2 and name = '".mysql_real_escape_string($_POST['txt_state'])."' and id != '".$_POST['hid_userid']."'";
	$result = $modelObj -> fetchRow($qry);
	...
	if($flag == 1)
	{
		$qry = "UPDATE regions SET name = '".clear_input($_POST['txt_state'])."', country_id = '".clear_input($_POST['txt_countrya'])."'  WHERE id = '".mysql_real_escape_string($_POST['hid_userid'])."'";
		$result = $modelObj->runQuery($qry);
		if($result)
		{
			echo "1";
...
if(isset($_POST['viewdiv']) && $_POST['viewdiv'] != ''){
	...
	if($_POST['search'] != '0')
	{
		...
		{
			$searchqry .="and s.name LIKE '%".mysql_real_escape_string(trim($_POST['txt_srchfname']))."%'";
			$_SESSION['srchqry'] =  $searchqry; 
		}
		if(trim($_POST['txt_country']) != '')
		{
			$searchqry .="and  cr.countryName LIKE '%".mysql_real_escape_string(trim($_POST['txt_country']))."%'";
			$_SESSION['srchqry'] =  $searchqry; 
		}
		...
		{
			$qry = "SELECT *,cr.name as countryName FROM countries as cr,regions as s WHERE s.country_id=cr.id and s.status != 2 ".$_SESSION['srchqry']."  order by s.id  asc LIMIT $start,$end ";
		}
		else
		{
			$qry = "SELECT *,cr.name as countryName FROM countries as cr,regions as s WHERE s.country_id=cr.id and s.status != 2 ".$_SESSION['srchqry']." ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
		}
		$qry11 = mysql_query("SELECT *,cr.name as countryName FROM countries as cr,regions as s WHERE s.country_id=cr.id and s.status != 2 ".$_SESSION['srchqry']."")or die(mysql_error());
	}
	else
	{
		if($_POST['fieldname'] == '0')
		{
			$qry = "SELECT *,cr.name as countryName FROM countries as cr,regions as s WHERE s.country_id=cr.id and s.status != 2 order by s.id desc LIMIT 0,$end ";/*$start,*/
		}
		else
		{
			$qry = "SELECT *,cr.name as countryName FROM countries as cr,regions as s WHERE s.country_id=cr.id and s.status != 2 ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
		}	  
		$qry11 = mysql_query("SELECT *,cr.name as countryName FROM countries as cr,regions as s WHERE s.country_id=cr.id and s.status != 2 ")or die(mysql_error());
	}
	//echo $qry; die;
...
<tr>
	<th class="table-header-check"><a id="toggle-all" ></a> </th>
		...
		<a onclick="sortingbyfield('s.name','<?php if($orderby=='asc'){echo "desc";}else{echo "asc";} ?>')" id="s_name" class="cursorpointer">State Name </a></th>
		<th class="table-header-repeat line-left minwidth-1">
		<a onclick="sortingbyfield('countryName','<?php if($orderby=='asc'){echo "desc";}else{echo "asc";} ?>')" id="cr_countryName" class="cursorpointer">Country Name </a></th>
...
?>
	...
	<?php 
	 ...
	 { 
	 ?>
		 <tr height="30"><td colspan="6" align="center" style="color:#FF0000"><strong><?php echo "No state found."; ?></strong></td></tr>
	 <?php 
	 }
	 ?>
	</table>
...
if(isset($_POST['delete']) && $_POST['delete'] != ''){
      $qry = "UPDATE regions SET status = 2 WHERE id = '".$_POST['id']."'";
	  $result = $modelObj->runQuery($qry);
	  $qry_city = "UPDATE tbl_city SET status = 2 WHERE stateid = '".$_POST['id']."'";
	  $result = $modelObj->runQuery($qry_city);
	  $qry_zip = "UPDATE tbl_zipcode SET status = 2 WHERE stateid = '".$_POST['id']."'";
	  $result = $modelObj->runQuery($qry_zip);
	  if($result){
	    echo $successmsg = '1';
	  }else{
...
if(isset($_POST['edit']) && $_POST['edit'] != ''){
	$qry = "SELECT * FROM regions WHERE id = '".$_POST['id']."'";
...
<form name="form_useradd" id="form_useradd" method="post" enctype="multipart/form-data" action="#" >
	<table border="0" cellpadding="0" cellspacing="0"  id="id-form" width="100%">
		<tr class="light_bg">
			<th>Country Name :</th>
			<td width="25%">
				<select class="select_box"  name="txt_countrya" id="txt_countrya" onblur="checkfield(this)">
			...
			<td id="errortxt_countrya" width="55%">
			<label class="removerror error_new" id="error-innertxt_countrya"></label>
            </td>
		</tr>
		<tr class="white_bg">
			<th>State Name :</th>
			<td><input type="text" class="input_box"  name="txt_state" id="txt_state" value="<?php echo stripslashes($data['name']) ?>" onblur="checkfield(this)" /><font class="required"> *</font></td>
