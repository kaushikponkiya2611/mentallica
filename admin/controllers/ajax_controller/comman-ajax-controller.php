...
//Payout
if(isset($_POST['payout_req']) && $_POST['payout_req'] != ''){
    
    //$qry = "UPDATE `tbl_wallet_payout_request` SET `request_status` = 'paid'  WHERE `user_id` = '".$_POST['artist_user_id']."'";
    $qry = "DELETE FROM `tbl_wallet_payout_request` WHERE `user_id` = '".$_POST['artist_user_id']."'";
    $result = $modelObj->runQuery($qry);
    
    $qry = "UPDATE `tbl_user_wallet` SET `amount` = '0'  WHERE `user_id` = '".$_POST['artist_user_id']."'";
    $result = $modelObj->runQuery($qry);
