<?php 
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:13px"><tr class="white_bg">
                      ...
                      <?php
                      if(stripslashes($result['categoryImage']!='')){
                          ?>
                        <tr class="light_bg">
                            <td width="120" align="right" class="popup_listing_border"><strong>Image:</strong></td>
                            <td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
                            <td width="469" align="left" class="popup_listing_border"><img src="<?=$_SESSION['SITE_NAME']?>upload/category/thumb/<?php echo stripslashes($result['categoryImage']) ?>" height="50" width="50" /></td>
                          </tr>
	...
	if(isset($_POST['hid_add']) && $_POST['hid_add'] != ''){
		...
		if($flag == 1)
		{
            ...
            if(isset($_FILES["txt_addcategoryImage"]["tmp_name"])){
                $tmpfile = $_FILES["txt_addcategoryImage"]["tmp_name"];
                $newname = $_FILES["txt_addcategoryImage"]["name"];
                $insertimage = "";
                if ($_FILES["txt_addcategoryImage"]["tmp_name"] != '') {
                    $abc = date("dmyHis");
                    $insertimage = $abc . $newname;
                   // echo $upload_dir . $insertimage;die;
            ...
            $qry = "INSERT INTO tbl_category (categoryName,categoryImage,cr_date,status) VALUES ('".clear_input($_POST['txt_addcategoryName'])."','".$insertimage."',NOW(),1)";
            $result = $modelObj->runQuery($qry);
			echo "1";
	   }
	   else
	...
	if(isset($_POST['hid_update']) && $_POST['hid_update'] != ''){
		$qry = "SELECT * FROM tbl_category WHERE categoryName = '".mysql_real_escape_string(trim($_POST['txt_addcategoryName']))."' and status!=2 and id != '".$_POST['hid_userid']."'";
		$result = $modelObj -> fetchRow($qry);
		...
		if($flag == 1)
		{
            ...
            if(isset($_FILES["txt_addcategoryImage"]["tmp_name"]))
            {
                ...
                if($_FILES["txt_addcategoryImage"]["tmp_name"] != '')
                {
                    ...
                    $qry_img="UPDATE tbl_category SET categoryImage='".$imgname."' where id='".mysql_real_escape_string($_POST['hid_userid'])."' ";
                    $res_img= $modelObj->runQuery($qry_img);
                }
            }
            
            $qry = "UPDATE tbl_category SET categoryName = '".clear_input($_POST['txt_addcategoryName'])."'WHERE id = '".mysql_real_escape_string($_POST['hid_userid'])."'";
			$result = $modelObj->runQuery($qry);
			echo "1";
		}
		else
	...
	$(function(){
		...
		$('#form_categoryview input[type=checkbox]').checkBox('toggle');
		return false;
		});
	});
	</script> 
	<form id="form_categoryview" action="" name="form_categoryview" method="post" enctype="multipart/form-data" onsubmit="return false">
	<div class="searchdiv">
	...
	<table border="0" width="100%" cellpadding="0" cellspacing="0" id="product-table">
		<tr>
		  <th class="table-header-check"><a id="toggle-all" ></a> </th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('categoryName','<?php if($orderby=='asc'){echo "desc";}else{echo "asc";} ?>')" id="categoryName" class="cursorpointer">CategoryName</a></th><th class="table-header-options line-left"><a>Options</a></th>
		</tr>
	<?php
	$i = 0;
	if($result != ''){
	...
	?>
		...
		<?php 
		 ...
		 ?>
			 <tr height="30">
			  <td colspan="7" align="center" style="color:#FF0000"><strong><?php echo "No category found."; ?></strong></td>
			</tr>
		 <?php 
		 }
		 ?>
    ...
    <form name="form_categoryadd" id="form_categoryadd" method="post" enctype="multipart/form-data" action="#" >
		<table border="0" cellpadding="0" cellspacing="0"  id="id-form" width="100%">
                    <tr class="white_bg">
						<th>CategoryName : </th>
						<td width="23%">
						<input class="input_box" type="text" name="txt_addcategoryName" id="txt_addcategoryName" value="<?php echo stripslashes($data['categoryName']) ?>" onblur="checkfield(this)"><font class="required"> *</font>
						</td>
                    ...
                    <tr class="white_bg">    
                        <th>CategoryImage : </th>
						<td width="23%">
						<input class="input_box" type="file" name="txt_addcategoryImage" id="txt_addcategoryImage" ><font class="required"> *</font>
						</td>
                        <?php
                        if($data['categoryImage']!=''){
                            ?>
                            <td id="errortxt_addimage" width="57%">
                                <img src="<?=$_SESSION['SITE_NAME']?>upload/category/thumb/<?php echo $data['categoryImage'] ?>" height="50" width="50" />
