<?php 
		...
		$upload_dir =$_SESSION['SITE_IMG_PATH']."advertisement/";
		$upload_dirthumb =$_SESSION['SITE_IMG_PATH']."advertisement/thumb/";
	
	if(isset($_POST['view']) && $_POST['view'] != ''){
		$id = $_POST['id'];
		$qry = "SELECT * FROM tbl_advertisement where id = '".mysql_real_escape_string($id)."'";
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:13px"><tr class="white_bg">
						...
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="469" align="left" class="popup_listing_border"><img src="<?=$_SESSION['SITE_NAME']?>upload/advertisement/thumb/<?php echo stripslashes($result['image']) ?>" height="50" width="50" /></td>
					  </tr></table>
	<?php 
	} 
	?>			
	...
	if(isset($_POST['statusactive']) && $_POST['statusactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_advertisement SET status = 1 WHERE id = ".mysql_real_escape_string($val);
	...
	if(isset($_POST['statusinactive']) && $_POST['statusinactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_advertisement SET status = 0 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
		...
		foreach($id as $k => $val)
		{
			$qry = "UPDATE  tbl_advertisement SET status = 2 WHERE id = ".mysql_real_escape_string($val);
			$result =  $modelObj->runQuery($qry);
		}
	}
	?>
	...
	if(isset($_POST['hid_add']) && $_POST['hid_add'] != ''){
		$qry = "SELECT * FROM tbl_advertisement WHERE title = '".mysql_real_escape_string(trim($_POST['txt_addtitle']))."' and status!=2 ";
		$result = $modelObj -> fetchRow($qry);
		...
		if($flag == 1)
		{
			...
			if(!dir($upload_dirthumb))
			{
				...
				}$qry = "INSERT INTO tbl_advertisement (title,cid,image,cr_date,status) VALUES ('".clear_input($_POST['txt_addtitle'])."','".clear_input($_POST['txt_addcat'])."','".$insertimage."',NOW(),1)";
			$result = $modelObj->runQuery($qry);
			echo "1";
	   }
	   else
	...
	if(isset($_POST['statusid']) && $_POST['statusid'] != ''){
		$qry = "UPDATE  tbl_advertisement SET status = ".mysql_real_escape_string($_POST['status'])." WHERE id = ".mysql_real_escape_string($_POST['statusid']);
		$result =  $modelObj->runQuery($qry);
	}
	?>
	<?php
	if(isset($_POST['hid_update']) && $_POST['hid_update'] != ''){
		$qry = "SELECT * FROM tbl_advertisement WHERE title = '".mysql_real_escape_string(trim($_POST['txt_addtitle']))."' and status!=2 and id != '".$_POST['hid_userid']."'";
		$result = $modelObj -> fetchRow($qry);
		...
		{if(isset($_FILES["txt_addimage"]["tmp_name"]))
				{
					...
					if($_FILES["txt_addimage"]["tmp_name"] != '')
					{
						...
						$qry_img="UPDATE  tbl_advertisement SET image='".$imgname."' where id='".mysql_real_escape_string($_POST['hid_userid'])."' ";
						$res_img= $modelObj->runQuery($qry_img);
					}
				}
				 $qry = "UPDATE tbl_advertisement SET title = '".clear_input($_POST['txt_addtitle'])."',cid = '".clear_input($_POST['txt_addcat'])."' WHERE id = '".mysql_real_escape_string($_POST['hid_userid'])."'"; 
			$result = $modelObj->runQuery($qry);
			echo "1";
		}
		else
	...
	if(isset($_POST['viewdiv']) && $_POST['viewdiv'] != ''){
		...
		{if(trim($_POST['txt_srctitle']) != '')
			...
			{
				$qry = "SELECT * FROM tbl_advertisement where status!=2 ".$_SESSION['srchqry']." order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_advertisement where status!=2 ".$_SESSION['srchqry']." ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}
			$qry11 = mysql_query("SELECT * FROM tbl_advertisement where status!=2 ".$_SESSION['srchqry']."")or die(mysql_error());
		}
		else
		{
			if($_POST['fieldname'] == '0')
			{
				$qry = "SELECT * FROM tbl_advertisement where status!=2 order by cr_date desc LIMIT $start,$end ";
			}
			else
			{
				$qry = "SELECT * FROM tbl_advertisement where status!=2 ORDER BY ".$_POST['fieldname']." ".$orderby." LIMIT $start,$end";
			}	  
			$qry11 = mysql_query("SELECT * FROM tbl_advertisement where status!=2 ")or die(mysql_error());
		}
	...
	$(function(){
		...
		$('#form_advertisementview input[type=checkbox]').checkBox('toggle');
		return false;
		});
	});
	</script> 
	<form id="form_advertisementview" action="" name="form_advertisementview" method="post" enctype="multipart/form-data" onsubmit="return false">
	<div class="searchdiv">
	...
	if(isset($_POST['delete']) && $_POST['delete'] != ''){
			$qry = "UPDATE tbl_advertisement SET status = 2 WHERE id = '".$_POST['id']."'";
			$result = $modelObj->runQuery($qry);
		  if($result){
			echo $successmsg = '1';
		  }else{
	...
	if(isset($_POST['edit']) && $_POST['edit'] != ''){
		$qry = "SELECT * FROM tbl_advertisement WHERE id = '".$_POST['id']."'";
	...
	?>
	<script type="text/javascript" src="<?=$LOCATION['SITE_ADMIN']?>views/javascripts/js/jquery.form.js"></script><form name="form_advertisementadd" id="form_advertisementadd" method="post" enctype="multipart/form-data" action="#" >
		...
		<tr class="white_bg">
					...
					<tr class="white_bg">
						<th>Category : </th>
						<td width="23%">
						<select name="txt_addcat" id="txt_addcat">
							<?php 
								$aqry = mysql_query("SELECT * FROM tbl_category WHERE status = 1");
								while($arow = mysql_fetch_array($aqry)){
							?>
									<option value="<?php echo $arow['id'];?>" <?php if($data['cid']==$arow['id']){ echo 'selected="selected"'; } ?>><?php echo $arow['categoryName'];?></option>
							<?php 
								}
							?>
						</select>
						...
						<td id="errortxt_addtitle" width="57%">
						<label class="removerror error_new" id="error-innertxt_addcat"></label>
						</td>
					</tr>
					<tr class="light_bg">
						<th>Image : </th>
						...
						<img src="<?=$_SESSION['SITE_NAME']?>upload/advertisement/thumb/<?php echo $data['image'] ?>" height="50" width="50" />
