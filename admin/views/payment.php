<script type="text/javascript">
	...
	<script type="text/javascript" src="<?=$LOCATION['SITE_ADMIN']?>views/javascripts/paymentscripts.js"></script>
	  ...
	  <table width="650" border="0" align="center" cellpadding="0" cellspacing="0">
		  <tr>
			<td width="635" height="35" class="popup_bg" ><table width="620" border="0" cellspacing="0" cellpadding="0" align="left">
				<tr>
				  <td colspan="2" >Upload Payment CSV</td>
				</tr>
			  </table></td>
		  </tr>
		  <tr>
			<td height="20" align="right" class="popup_listing_border"><a href="<?=$LOCATION['SITE_ADMIN']?>csv-examples/payment.csv">Example of CSV Format</a></td>
		  </tr>
		  <tr>
				...
				<form id="form_search" action="" name="form_search" method="post" enctype="multipart/form-data" >
				  <table width="100%" border="0" cellspacing="0" cellpadding="0">
					  <tr>
						...
						<input type="text" class="input_box"  name="txt_srcuser_id" id="txt_srcuser_id" />	  
						</td>
						...
						<td width="130" align="right" class="popup_listing_border" valign="middle"><strong>Transaction Id:</strong></td>
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="459" align="left" class="popup_listing_border" valign="middle">
						<input type="text" class="input_box"  name="txt_srctransaction_id" id="txt_srctransaction_id" />	  
						</td>
					  </tr><tr class="light_bg">
						<td width="130" align="right" class="popup_listing_border" valign="middle"><strong>Amount:</strong></td>
						<td width="11" height="37" align="left" class="popup_listing_border">&nbsp;</td>
						<td width="459" align="left" class="popup_listing_border" valign="middle">
						<input type="text" class="input_box"  name="txt_srcamount" id="txt_srcamount" />	  
						</td>
					  </tr><tr class="white_bg">
						<td align="right">&nbsp;</td>
	  ...
	  <div id="<?php echo $_GET['pid'] ?>" >
		<div class="searchdiv">
		  <table class="searchdiv" border="0" width="100%" cellpadding="0" cellspacing="0">
			...
			/*if($controller_class -> getpayment != ''){
			?>
		...
		<form id="form_paymentview" action="" name="form_paymentview" method="post" enctype="multipart/form-data" >
		  <table border="0" width="100%" cellpadding="0" cellspacing="0" id="product-table">
			<tr>
			  <th class="table-header-check"><a id="toggle-all" ></a> </th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('user_id','asc')" id="user_id" class="cursorpointer">User Name</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('plan_id','asc')" id="plan_id" class="cursorpointer">Plan Name</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('transaction_id','asc')" id="transaction_id" class="cursorpointer">Transaction Id</a></th><th class="table-header-repeat line-left minwidth-1"> <a onclick="sortingbyfield('amount','asc')" id="amount" class="cursorpointer">Amount</a></th><th class="table-header-options line-left"><a>Options</a></th>
			</tr>
			<?php
	   $i = 0;
		if($controller_class -> getpayment != ''){
			foreach($controller_class -> getpayment  as $k => $data){
				$i++;
				$username = $controller_class -> getusernamebyid($data['user_id']);
				$planname = $controller_class -> getplannamebyid($data['plan_id']);
	  ?>
			<tr id="<?php echo $data['id']?>" class="<?php if($i%2==0){ echo "light_bg"; }else{ echo "white_bg"; } ?>" height="30">
			  <td><input  type="checkbox" name="chk_id" id="chk_id" value="<?php echo $data['id'];  ?>"/></td><td class="cursorpointer" onclick="edit('<?php echo $data['id'] ?>','<?php echo $_GET['pid']?>')"><?php echo $username['first_name']." ".$username['last_name']; ?></td><td><?php echo $planname['plan_name']; ?></td><td><?php echo $data['transaction_id']; ?></td><td><?php echo $data['amount']; ?></td><td class="options-width" ><table>
				  <tr>
					<!--<td><?php
							if($data['status'] == '1')
							{
