function searching(){
	...
	$("#searchmsg").html('');
	var flag = 0;
	var regflag = 0;if($("#txt_srctitle").val() == ''){
		flag = 1;
	}
	if(flag == 1){
	...
	else
	{
		if(regflag == 0)
		{
			...
			var options = {
				...
				url:       site_url+'controllers/ajax_controller/sponser-ajax-controller.php', 
				type: "POST"
			};
          ...
          }else
	{
		...
		$('#form_sponseradd').submit(function() {
			$(this).ajaxSubmit(options);
			return false;
		});
	}
	...
	if(statusText == 'success')
	{
		...
		if(data == 0)
		{
			...
			$("#message-green").hide();
			document.getElementById('err').innerHTML = 'sponser already exist. Please try another.';
		}else if(data == 1){
			$("#message-red").hide();
			$("#message-green").show().fadeOut(5000);		   
			document.getElementById('succ').innerHTML = 'sponser added successfully.';
			newdata();				 
		}else if(data == 2){
			$.scrollTo(0,500);
			$("#message-red").show().fadeOut(7000);
			$("#message-green").hide();
			document.getElementById('err').innerHTML = 'Some error occurred while adding sponser.';
		}else if(data == 3){
                    
                    $.scrollTo(0,500);
			$("#message-red").show().fadeOut(7000);
			$("#message-green").hide();
			document.getElementById('err').innerHTML = 'Image size should be (400*160).';
                        
                        
			
							 
		}
		$('#form_sponseradd').unbind('submit').bind('submit',function() {
		});
	}
	}
	function updatedata(){
	...
	function showResponse_update(data, statusText)  
	{
		if (statusText == 'success') 
		{
			if(data == 0){
				...
				$("#message-red").hide();
				$("#message-green").show().fadeOut(5000);				
				document.getElementById('succ').innerHTML = 'sponser updated successfully.';
				newdata();
			}else if(data == 2){
				$.scrollTo(0,500);
				$("#message-red").show().fadeOut(7000);
				$("#message-green").hide();
				document.getElementById('err').innerHTML = 'Some error occurred while updating sponser.';
			}
		}
		$('#form_sponseradd').unbind('submit').bind('submit',function() {
		});
