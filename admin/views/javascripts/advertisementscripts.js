function searching(){
	...
	else
	{
		if(regflag == 0)
		{
			...
			var options = {
				...
				url:       site_url+'controllers/ajax_controller/advertisement-ajax-controller.php', 
				type: "POST"
			};
				...
				}else
	{
		...
		$('#form_advertisementadd').submit(function() {
			$(this).ajaxSubmit(options);
			return false;
		});
	}
	...
	if(statusText == 'success')
	{
		if(data == 0)
		{
			...
			$("#message-green").hide();
			document.getElementById('err').innerHTML = 'Advertisement already exist. Please try another.';
		}else if(data == 1){
			$("#message-red").hide();
			$("#message-green").show().fadeOut(5000);		   
			document.getElementById('succ').innerHTML = 'Advertisement added successfully.';
			newdata();				 
		}else if(data == 2){
			$.scrollTo(0,500);
			$("#message-red").show().fadeOut(7000);
			$("#message-green").hide();
			document.getElementById('err').innerHTML = 'Some error occurred while adding advertisement.';
		}
		$('#form_advertisementadd').unbind('submit').bind('submit',function() {
		});
	}
	}
	function updatedata(){
	...
	function showResponse_update(data, statusText)  
	{
		if (statusText == 'success') 
		{
			if(data == 0){
				...
				$("#message-red").hide();
				$("#message-green").show().fadeOut(5000);				
				document.getElementById('succ').innerHTML = 'Advertisement updated successfully.';
				newdata();
			}else if(data == 2){
				$.scrollTo(0,500);
				$("#message-red").show().fadeOut(7000);
				$("#message-green").hide();
				document.getElementById('err').innerHTML = 'Some error occurred while updating advertisement.';
			}
		}
		$('#form_advertisementadd').unbind('submit').bind('submit',function() {
		});
